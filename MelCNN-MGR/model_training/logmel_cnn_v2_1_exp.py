# %% [markdown]
# # Log-Mel CNN v2.1-exp
#
# Experimental branch of `logmel_cnn_v2_1.py` targeting higher Macro-F1 via:
# - **Softer SpecAugment sizes** (freq=16, time=25, num_masks=1) — test whether 24/40 still masks too much signal
# - **Graduated spatial dropout** (0.05→0.20) — concentrate regularization in deeper layers
# - **Reduced deepest spatial dropout** (block5: 0.20→0.10) — test whether the last block was over-regularized
...
# python MelCNN-MGR/model_training/logmel_cnn_v2_1_exp.py --pretrained-model MelCNN-MGR/models/logmel-cnn-demo/best_model_macro_f1.keras
# python MelCNN-MGR/model_training/logmel_cnn_v2_1_exp.py --pretrained-model MelCNN-MGR/models/logmel-cnn-demo/best_model_macro_f1.keras --pretrained-mode backbone_only --freeze-backbone-epochs 3
...
CLI_PARSER = argparse.ArgumentParser(
    description="Train Log-Mel CNN v2.1-exp from scratch or warm-start from an existing .keras checkpoint.",
)
...
RUN_DIR = None
RUN_FAMILY = "logmel-cnn-v2_1-exp"
...
SPATIAL_DROPOUT_RATE_BLOCK5 = 0.10  # v2.1-exp: experimental softer deepest-block dropout (was v2.1: 0.20)
...
def build_model(n_classes: int) -> keras.Model:
    ...
    return keras.Model(inputs, outputs, name="logmel_cnn_v2_1_exp")
...
model_path = RUN_DIR / "logmel_cnn_v2_1_exp.keras"
...
fig.suptitle("Training history — Log-Mel CNN v2.1-exp")
plt.tight_layout()
plt.show()
...
ax.set_title("Confusion matrix — test set (logmel_cnn_v2_1_exp)")
plt.tight_layout()
plt.show()
...
report = {
    ...
    "changes_from_v2_1": {
        "experiment_goal": "test whether softer SpecAugment sizes and lower deepest-block spatial dropout improve accuracy / Macro-F1",
        "weight_decay": "1e-4 (was 5e-4) — lighter L2 with Mixup+dropout active",
        "spec_aug_mask_sizes": "freq=16, time=25, num_masks=1 (was v2.1: freq=24, time=40, num_masks=1) — experimental softer masking",
        "spatial_dropout": "0.05/0.10/0.15/0.10 (was v2.1: 0.05/0.10/0.15/0.20) — experimental softer deepest-block dropout",
...
report_path = RUN_DIR / "run_report_logmel_cnn_v2_1_exp.json"
