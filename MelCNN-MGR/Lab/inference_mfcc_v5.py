"""
MFCC CNN v5 Inference Module
============================
Reusable inference engine for the MFCC 2D-CNN baseline genre classifier
trained by ``baseline_mfcc_cnn_v5.ipynb``.

Loads a trained model, normalization stats, and genre labels from a run
directory produced by the training notebook.  Supports single-crop
(full 30 s) and 3-crop (early / middle / late) inference.

Run directory layout (produced by the training notebook)::

    mfcc-cnn-YYYYMMDD-HHMMSS/
        baseline_mfcc_cnn.keras   # trained Keras model
        norm_stats.npz            # mu, std, genre_classes
        run_report_<subset>.json  # training report (optional)

Usage
-----
    from MelCNN_MGR.inference_mfcc_v5 import MFCCCNNInference
...
# ── Audio / MFCC constants (must match training) ──────────────────────────────
SAMPLE_RATE   = 22_050
N_MFCC        = 13
N_FFT         = 512
HOP_LENGTH    = 256
CLIP_DURATION = 30.0                                              # seconds
N_FRAMES      = 2582   # 1 + (661500 - 512) // 256 for 30 s
MFCC_SHAPE    = (N_MFCC, N_FRAMES)
...
class PredictionResult:
    """Result returned by ``MFCCCNNInference.predict()``."""
    file: str
    genre: str
    confidence: float
    probs: list[float]
...
def extract_mfcc(y: np.ndarray) -> np.ndarray:
    """Compute a fixed-shape ``(13, 2582)`` MFCC feature matrix."""
...
def extract_three_crops(
    ...
    """Extract 3 deterministic 30-second waveform crops (early / middle / late).

    - clips > 3 x target: crops centered at 25 %, 50 %, 75 % of duration
    - clips between 1x and 3x target: start / center / end
...
class MFCCCNNInference:
    """Load a trained MFCC CNN v5 model and run genre inference on audio files.
    ...
    def __init__(self, run_dir: str | Path) -> None:
        ...
        self.mu: np.ndarray = data["mu"]            # (1, 13, 1, 1)
        self.std: np.ndarray = data["std"]           # (1, 13, 1, 1)
    ...
    def _normalize_mfcc(self, mfcc: np.ndarray) -> np.ndarray:
        """Apply z-normalization and reshape for model input."""
        x = mfcc[np.newaxis, ..., np.newaxis]          # (1, 13, 2582, 1)
        x = ((x - self.mu) / self.std).astype(np.float32)
        return x

    def _predict_mfcc(self, mfcc: np.ndarray) -> np.ndarray:
        """Run model on a single normalized MFCC, return probability vector."""
        x = self._normalize_mfcc(mfcc)
    ...
    def predict(
        ...
        mode : str
            ``"single_crop"`` (default) — center 30 s clip only.
        ...
        if mode == "single_crop":
            ...
            mfcc = extract_mfcc(y_clip)
            probs = self._predict_mfcc(mfcc)
        ...
        crop_mfccs = [extract_mfcc(c) for c in crops]

        crop_probs = [self._predict_mfcc(m) for m in crop_mfccs]
