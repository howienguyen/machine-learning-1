...
class _MacroF1Checkpoint(tf.keras.callbacks.Callback):
    """Compute val macro-F1 at end of select epochs; save model when it improves.
    ...
    Frequency strategy (three-phase):
      - epoch < mid_freq_from_epoch  : run every ``check_freq`` epochs  (sparse; fast-changing early training)
      - mid_freq_from_epoch <= epoch
        < dense_from_epoch           : run every ``mid_freq`` epochs     (medium; approaching convergence)
      - epoch >= dense_from_epoch    : run every epoch                   (dense; fine-grained near convergence)

    ``f1_history`` always has one entry per epoch (last known value is repeated on
    skipped epochs) so the list is epoch-aligned for plotting.
    """
    def __init__(self, val_ds: tf.data.Dataset, genre_classes: list, filepath: str,
                 check_freq: int = 3, mid_freq: int = 2, mid_freq_from_epoch: int = 30,
                 dense_from_epoch: int = 60):
        super().__init__()
        self.val_ds               = val_ds
    ...
    def _should_check(self, epoch: int) -> bool:
        """Return True if this epoch should run the full macro-F1 evaluation."""
...
_f1_ckpt      = _MacroF1Checkpoint(
    ...
    check_freq=3,              # sparse phase  : every 3 epochs  (epochs  0–29)
    mid_freq=2,                # medium phase  : every 2 epochs  (epochs 30–59)
    mid_freq_from_epoch=30,
    dense_from_epoch=60,       # dense phase   : every epoch     (epoch  60+)
)
...
if _best_f1_path.exists():
    ...
    print("  (deserializing weights + optimizer — this may take 15–30 s) ...", flush=True)
    import threading as _threading
    _load_done = _threading.Event()
    def _dot_printer():
        while not _load_done.wait(timeout=9.0):
            print("  ...", flush=True)
    _dot_thread = _threading.Thread(target=_dot_printer, daemon=True)
    _dot_thread.start()
...
if INFER_PATHS is None:
    ...
    for _p, _g in zip(INFER_PATHS, _true_genres):
        print(f"Using test sample: {_p}  (true genre: {_g})")
else:
