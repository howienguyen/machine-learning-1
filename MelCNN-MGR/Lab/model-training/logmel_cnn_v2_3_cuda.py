...
# # Log-Mel CNN v2.3
#
# Per-mel-bin standardization upgrade of `logmel_cnn_v2_2_cuda.py` that keeps
# the same backbone, staged training, and regularization recipe while making
# the normalization experiment explicit and reproducible.
#
...
# python MelCNN-MGR/model_training/logmel_cnn_v2_3_cuda.py
#
# Warm-start usage example:
# python MelCNN-MGR/model_training/logmel_cnn_v2_3_cuda.py --pretrained-model MelCNN-MGR/models/logmel-cnn-demo/best_model_macro_f1.keras
# python MelCNN-MGR/model_training/logmel_cnn_v2_3_cuda.py --pretrained-model MelCNN-MGR/models/logmel-cnn-demo/best_model_macro_f1.keras --pretrained-mode backbone_only --freeze-backbone-epochs 3
...
CLI_PARSER = argparse.ArgumentParser(
    description="Train Log-Mel CNN v2.3 with train-only per-mel-bin standardization and gap-aware early stopping from scratch or warm-start from an existing .keras checkpoint.",
)
...
DATASET_NUM_PARALLEL_CALLS = 1      # fixed tf.data map parallelism; avoids AUTOTUNE-side buffer growth
DATASET_PREFETCH_BUFFER_SIZE = 1    # fixed single-batch prefetch; avoids prefetch autotuner RAM expansion
...
def compute_train_per_mel_bin_stats(index_df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i, path in enumerate(index_df["logmel_path"]):
        x = np.load(path).astype(np.float64, copy=False)
        if x.ndim != 2 or x.shape[0] != N_MELS:
            raise ValueError(f"Unexpected log-mel shape for {path}: {x.shape}; expected ({N_MELS}, n_frames).")
        sum_per_bin += x.sum(axis=1)
        count_per_bin += x.shape[1]
        if (i + 1) % 2000 == 0:
            print(f"  stats pass 1/2: {i + 1}/{len(index_df)}")
    ...
    for i, path in enumerate(index_df["logmel_path"]):
        ...
        if (i + 1) % 2000 == 0:
            print(f"  stats pass 2/2: {i + 1}/{len(index_df)}")
...
def make_dataset(index_df: pd.DataFrame, batch_size: int, shuffle: bool, augment: bool = False, weighted_mixup: bool = False) -> tf.data.Dataset:
    ...
    ds = ds.map(_load_and_norm, num_parallel_calls=DATASET_NUM_PARALLEL_CALLS)
...
def build_model(n_classes: int) -> keras.Model:
    ...
    return keras.Model(inputs, outputs, name="logmel_cnn_v2_3")
...
model_path = RUN_DIR / "logmel_cnn_v2_3.keras"
...
ax.set_title("Confusion matrix — test set (logmel_cnn_v2_3)")
plt.tight_layout()
plt.show()
...
report_path = RUN_DIR / "run_report_logmel_cnn_v2_3.json"
