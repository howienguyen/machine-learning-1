...
LOGMEL_DATASET_DIR = Path(
    os.environ.get(
        "LOGMEL_DATASET_DIR",
        str(CACHE_DIR / "logmel_dataset_v21_10s"),
    )
)
...
class _MacroF1Checkpoint(tf.keras.callbacks.Callback):
    def __init__(self, val_ds: tf.data.Dataset, filepath: str, check_freq: int = 3, mid_freq: int = 2, mid_freq_from_epoch: int = 30, dense_from_epoch: int = 60):
        super().__init__()
        self.val_ds = val_ds
