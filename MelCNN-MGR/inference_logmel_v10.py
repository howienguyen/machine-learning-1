"""
Log-Mel CNN v10 Inference Module
==================================
Reusable inference engine for Log-Mel CNN v10 genre classification.

Loads a trained model, normalization stats, and genre labels from a run
directory produced by ``baseline_logmel_cnn_v10.ipynb``.  Supports both
single-crop (center 30 s) and 3-crop (early/middle/late) inference.

Run directory layout (produced by the training notebook)::

    logmel-cnn-v10-YYYYMMDD-HHMMSS/
        baseline_logmel_cnn_v10.keras   # trained Keras model
        norm_stats.npz                  # mu, std, genre_classes
        run_report_<subset>.json        # training report (optional)

Usage
-----
    from MelCNN_MGR.inference_logmel_v10 import LogMelV10Inference
...
N_FRAMES      = int(CLIP_DURATION * SAMPLE_RATE / HOP_LENGTH)  # 2582
...
class PredictionResult:
    """Result returned by ``LogMelV10Inference.predict()``."""
    file: str
    genre: str
    confidence: float
    probs: list[float]
...
def extract_logmel(y: np.ndarray) -> np.ndarray:
    """Compute a fixed-shape ``(128, 2582)`` log-mel spectrogram."""
...
class LogMelV10Inference:
    """Load a trained v10 model and run genre inference on audio files.
    ...
    def __init__(self, run_dir: str | Path) -> None:
        ...
        # ── Load model ────────────────────────────────────────────────────
        model_path = run_dir / "baseline_logmel_cnn_v10.keras"
    ...
    def _normalize_logmel(self, logmel: np.ndarray) -> np.ndarray:
        ...
        x = logmel[np.newaxis, ..., np.newaxis]          # (1, 128, 2582, 1)
