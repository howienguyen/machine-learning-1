"""
MelCNN-MGR Inference Module
============================
Reusable inference engine for Log-Mel CNN v20a genre classification.

Loads a trained model, normalization stats, and genre labels from a run
directory produced by ``baseline_logmel_cnn_v20a.py``.  Supports both
single-crop (center 10 s) and 3-crop (early/middle/late) inference.

Run directory layout (produced by the training script)::

    logmel-cnn-v20a-YYYYMMDD-HHMMSS/
        baseline_logmel_cnn_v20a.keras   # trained Keras model
        norm_stats.npz                   # mu, std, genre_classes
        run_report_<subset>.json         # training report (optional)

Usage
-----
    from MelCNN_MGR.inference_logmel_v20a import MelCNNInference

    engine = MelCNNInference("MelCNN-MGR/models/logmel-cnn-v20a-20260308-013615")
...
class MelCNNInference:
    """Load a trained v20a model and run genre inference on audio files.
